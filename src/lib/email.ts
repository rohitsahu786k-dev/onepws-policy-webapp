...
const adminNotificationEmail = process.env.ADMIN_NOTIFICATION_EMAIL || 'marketing@onepws.com';
...
export async function sendApprovalStatusEmail(user: { name: string; email: string; status: string }) {
  ...
  const subject = isApproved
    ? 'Your policy portal account is approved'
    : user.status === 'rejected'
      ? 'Your policy portal access request was rejected'
      : user.status === 'blocked'
        ? 'Your policy portal account has been blocked'
        : 'Your policy portal account is pending review';
  ...
  const message = isApproved
    ...
    : user.status === 'rejected'
      ...
      : user.status === 'blocked'
        ? 'Your account has been blocked and can no longer access the policy portal.'
  ...
  return sendEmail({
    ...
    html: renderEmailTemplate({
      title: subject,
      eyebrow: 'Policy Hub account update',
...
export async function sendUserCreatedEmail(user: { name: string; email: string; password: string; status: string }) {
  return sendEmail({
    to: user.email,
    subject: 'Your policy portal account has been created',
    html: renderEmailTemplate({
      title: 'Your account is ready',
      eyebrow: 'Policy Hub access',
      body: 'An administrator created a policy portal account for you. Sign in with the temporary password below and change it from your profile page.',
      rows: [
        ['Email', user.email],
...
export async function sendUserRemovedEmail(user: { name: string; email: string }) {
  return sendEmail({
    to: user.email,
    subject: 'Your policy portal account was removed',
    html: renderEmailTemplate({
      title: 'Account removed',
      eyebrow: 'Policy Hub account update',
      body: `Hello ${user.name}, your policy portal account has been removed by an administrator.`,
    }),
  });
}
...
export async function sendPasswordResetEmail(user: { name: string; email: string; password: string }) {
  return sendEmail({
    to: user.email,
    subject: 'Your policy portal password was reset',
    html: renderEmailTemplate({
      title: 'Password reset completed',
      eyebrow: 'Policy Hub security update',
      body: 'An administrator reset your policy portal password. Sign in with the temporary password below and change it from your profile page.',
      rows: [
        ['Email', user.email],
        ['Temporary password', user.password],
      ],
...
export async function sendForgotPasswordEmail(user: { name: string; email: string; resetLink: string }) {
  const bodyHtml = `
    <p style="margin:0 0 18px;font-size:15px;line-height:1.7;color:#374151">
      Hello ${escapeHtml(user.name)}, you requested to reset your policy portal password. Click the button below to create a new password. This link expires in 1 hour.
    </p>
  ...
  return sendEmail({
    to: user.email,
    subject: 'Reset your policy portal password',
    html: `
      <div style="margin:0;padding:0;background:#f3f4f6;font-family:Arial,sans-serif;color:#111827">
        <div style="max-width:640px;margin:0 auto;padding:32px 18px">
          <div style="background:#111827;border-radius:8px 8px 0 0;padding:22px 26px;color:#ffffff">
            <div style="font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#f87171;font-weight:700">Policy Hub security update</div>
          ...
          <div style="background:#ffffff;border:1px solid #e5e7eb;border-top:0;border-radius:0 0 8px 8px;padding:26px">
            ${bodyHtml}
            <p style="margin:22px 0 0;font-size:12px;color:#6b7280">This is an automated email from OnePWS Policy Hub.</p>
          </div>
        </div>
      </div>
    `,
